...
main <- function(){
  ...
  if(!no_motif){
    ...
    load("../data/shareseq/mouse_pwms_v2.rda")
    motif_ix_scores <- matchMotifs(mouse_pwms_v2, peaks_seq, genome = BSgenome.Mmusculus.UCSC.mm10, out = "scores")    
    ...
    motif_names = names(mouse_pwms_v2)
  ...
  if(!no_motif){
    ...
    # system(paste0('mkdir -p ',output_dir))
    filename = 'matrix.mtx'
    writeMM(freq_motif,file = file.path(dir.output,filename))
    write.table(rownames(freq_motif),file.path(dir.output,'peaks.tsv'),quote=FALSE,row.names = FALSE,col.names = FALSE)
    write.table(colnames(freq_motif),file.path(dir.output,'motifs.tsv'),quote=FALSE,row.names = FALSE,col.names = FALSE)

    #filename = 'freq_motif.h5'
    ## writeHDF5Array internally transposes the matrix so `t()` is used to counteract this operation
    #writeHDF5Array(t(freq_motif), file.path(dir.output,filename), name="mat_motif", with.dimnames=TRUE, verbose=TRUE)
    ## using this structure in order for  anndata 'read_hdf' to recognize row names and column names
